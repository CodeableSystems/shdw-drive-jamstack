...
async function getListObjects(account) {
	try {
		const resp = await axios.post(`https://shadow-storage.genesysgo.net/list-objects-and-sizes`, {
...
gulp.task('upload', function () {
	return new Promise(function (resolve) {
		gulp.src(['build'], { read: false })
			.pipe(using())
			.pipe(shell([`BROWSER="" shdw-drive upload-multiple-files -kp ${process.env.WALLETFILE} -s ${process.env.DRIVE} -d <%= file.path %>`], { ignoreErrors: true }))
		resolve()
	});
})
	...
	return new Promise(function (resolve) {
		gulp.src(['build/**/*'], { read: false })
			...
			.pipe(through.obj(async (chunk, enc, cb) => {
				...
				let uploadedFile = remoteFiles.files.filter(f => f.file_name === chunk.relative)
